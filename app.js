...
http.createServer((req, res) => {
	...
	switch (req.url) {
			...
			movies = movies.sort(function (a, b){
				...
				return 0;
			     ...
			     for (var i = 0 ; i < movies.length ; i++){
			         ...
			         let content= `Bienvenidos a DH Movies el mejor sitio para encontrar las mejores <br>
			         películas, incluso mucho mejor que Netflix, Cuevana y PopCorn.<br>
					 <hr/>
					 <br>
			         Total de peliculas en Cartelera ${movies.length}<br>
					 <br>
		...
		case '/en-cartelera':
				...
				let listadoEnCartelera =``
				 
			        for (var i = 0 ; i < movies.length ; i++){
						listadoEnCartelera += ` <b>- Titulo:</b> ${movies[i].title}  <br>
						<b>- Reseña:</b> ${movies[i].overview}<br>
						<br>`
			         }
				     let contentEnCartelera=`
			         <b>En cartelera </b><br>
			         <br>
		...
		case '/mas-votadas':
				 ...
				 for (var i = 0 ; i < movies.length ; i++){
				   sumaTodo+=movies[i].vote_average
				 }

				 let aPromedio= sumaTodo/movies.length
				 
			     for (var i = 0 ; i < movies.length ; i++){
					if (`${movies[i].vote_average}`>=aPromedio){
					masVotadas += ` <b>- Titulo: </b> ${movies[i].title}  <br>
									<b>- Rating: </b> ${movies[i].vote_average}<br>
									<b>- reseña: </b> ${movies[i.overview]}<br>
									<br>`
					 }}
					 let votadas=`
			...
			break;
		case '/sucursales':
				let listadoSucursales =``
				 
			for (var i = 0 ; i < theaters.length ; i++){
				listadoSucursales += ` <b>- Nombre:</b> ${theaters[i].name}  <br>
				<b>- Direccion:</b> ${theaters[i].address}<br>
                <b>- Descripcion:</b> ${theaters[i].description}<br>
				<br>`
			 }
			 let contentSucursales=`
			 <b>Sucursales </b><br>
			 <br>
			 Total de sucursales ${theaters.length}<br>
			 <br>
			...
			break;
		case '/contacto':
			res.end(`<b>Contacto:</b><br>
			 <u>¿Tenés algo para contarnos? </u><br>Nos encanta escuchar a nuestros
              clientes. <br> Si deseas contactarnos podés escribirnos al siguiente email:
              <b>dhmovies@digitalhouse.com</b> o en las redes sociales. <br> Envianos tu consulta,
              ...
              respuestas inmediatas a los problemas más comunes.<br>
              <a href="/"> Volver a Home</a><br>
              `);
			break;
		...
		for (var i = 0 ; i < faqs.length ; i++){
			preguntasFrecuentes += ` <b>- Pregunta:</b> ${faqs[i].faq_title}  <br>
			<b>- Respuesta:</b> ${faqs[i].faq_answer}<br>
			<br>`
		 }
		 let contentFaqs=`
